...
<iframe src="https://ats.rankmi.com/tenants/769/organizations/skyway-equities/get_new_iframe" width="100%" height="400" frameBorder="0"></iframe>
...
function openCotizModal(d) {
    ...
    document.getElementById("cotiz-saludo").textContent = "Hola " + primer + ", aquí tienes tu cotización";
    ...
    // CTAs: WhatsApp + Llamar
    const waNum = c.asesor_wa || '';
    const prop  = c.propiedad || 'Park Life';
    const precio = fmt(c.precio_desde);
    const waMsg = `Hola ${asesor}, vi la cotización de ${prop} desde ${precio}/mes y me interesa. ¿Podemos hablar?`;
    const waBtn = document.getElementById('cotiz-wa-btn');
    const callBtn = document.getElementById('cotiz-call-btn');
    if (waBtn && waNum) {
        waBtn.href = 'https://wa.me/' + waNum.replace(/[^0-9]/g,'') + '?text=' + encodeURIComponent(waMsg);
    }
    if (callBtn && waNum) {
        callBtn.href = 'tel:+' + waNum.replace(/[^0-9]/g,'');
    }
